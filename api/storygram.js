...
try {
  ...
  GlobalFonts.register(
    fs.readFileSync(path.join(process.cwd(), "fonts/Inter-Bold.ttf")),
    "InterBold"
  );
  GlobalFonts.register(
    fs.readFileSync(path.join(process.cwd()), "fonts/Inter-SemiBold.ttf")),
    "InterSemiBold"
  );
  ...
  try {
    ...
    hasEmoji = true;
    console.log("✅ Registered fonts:", GlobalFonts.families.join(", "));
  } catch (emojiErr) {
    console.warn("⚠️ Emoji font not loaded:", emojiErr.message);
  }
} catch (e) {
  console.error("❌ FONT ERROR:", e.message);
}

// ── Font helpers (TANPA kutip & sans-serif) ─────────────────────────────
function FN(sz) {
  return `normal ${sz}px ${hasEmoji ? "Inter, NotoColorEmoji" : "Inter"}`;
}
function FB(sz) {
  return `bold ${sz}px ${hasEmoji ? "InterBold, NotoColorEmoji" : "InterBold"}`;
}
function FSB(sz) {
  return `600 ${sz}px ${hasEmoji ? "InterSemiBold, NotoColorEmoji" : "InterSemiBold"}`;
}
...
async function drawAvatar(ctx, img, cx, cy, r) {
  ...
  if (img) {
    ...
    g.addColorStop(0, "#34354a");
    g.addColorStop(1, "#1a1a24");
    ctx.fillStyle = g;
    ctx.fillRect(cx - r, cy - r, r * 2, r * 2);
    ctx.fillStyle = "rgba(255,255,255,0.20)";
    ctx.beginPath();
    ctx.arc(cx, cy - r * 0.1, r * 0.36, 0, Math.PI * 2);
    ctx.fill();
    ctx.beginPath();
    ctx.ellipse(cx, cy + r * 0.62, r * 0.48, r * 0.30, 0, 0, Math.PI * 2);
    ctx.fill();
  }
  ctx.restore();
}
...
function icoReport(ctx, cx, cy, s, color) {
  ...
  ctx.lineJoin = "round";
  ctx.beginPath();
  ctx.arc(cx, cy, s * 0.88, 0, Math.PI * 2);
  ctx.stroke();
  ctx.beginPath();
  ...
  ctx.stroke();
  ctx.beginPath();
  ctx.arc(cx, cy + s * 0.44, s * 0.10, 0, Math.PI * 2);
  ctx.fill();
  ctx.restore();
}
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    topHL.addColorStop(0, "rgba(255,255,255,0.10)");
    ...
    // ── Username (menggunakan chatFont agar emoji di username tampil) ──
    if (username) {
      const maxDispLen = 22;
    ...
    // ── Caption (sudah pakai chatFont, tapi kita pastikan) ──
    ...
    ctx.stroke();
    ctx.beginPath();
    ctx.moveTo(mx + 9, my - 4);
    ctx.lineTo(mx + 14, my + 4);
    ctx.stroke();
    ctx.beginPath();
    ctx.moveTo(mx + 14, my - 4);
