...
// ── SHARED HELPERS (same as quotedchat.js) ────────────────────
...
// Draw an image into a rect like CSS object-fit: cover
...
// Cheap blur: render small, then scale up
...
// ── ICONS: simple stroke-based line icons ──────────────────────
function iconHeart(ctx, cx, cy, s, filled, color) {
  ctx.save();
  ...
  ctx.bezierCurveTo(cx - s*1.05, cy - s*0.15, cx - s*0.55, top - s*0.55, cx, top + s*0.05);
  ctx.bezierCurveTo(cx + s*0.55, top - s*0.55, cx + s*1.05, cy - s*0.15, cx, cy + s*0.62);
  ctx.closePath();
...
const ICONS = {
  like:    (ctx, x, y, s, c, active) => iconHeart(ctx, x, y, s, !!active, c),
...
/**
 * Query params:
 *  avatar       - URL foto profil pemilik story (opsional)
 *  avatarColor  - warna ring avatar, hex (default "#22d3ee")
 *  media        - URL gambar konten story (opsional)
 *  mediaColor   - warna placeholder kalau media kosong (default "#22c55e")
 *  background   - URL gambar latar, akan diblur otomatis (opsional)
 *  username     - nama pengguna di samping avatar (opsional)
 *  liked        - "true"/"false", heart terisi merah kalau true (default false)
 *  menu         - daftar item dipisah koma, subset dari like,comment,repost,share,report
 *                 (default: kelimanya, urut seperti default)
 *  likes,comments,reposts,shares - angka opsional, tampil di sebelah label
 *  width,height - ukuran kanvas (default 900x1430)
 *
 * Contoh:
 *  /api/storygram?avatar=<url>&media=<url>&username=Farel&liked=true&likes=128
 */
module.exports = async (req, res) => {
  ...
  try {
    ...
    const HEAD_H = Math.round(cH * 0.105);
    ...
    // Clip to card while drawing header bg + media
    ctx.save();
    ...
    // Avatar + username (drawn after the clip is released)
    const AV_R  = Math.round(HEAD_H * 0.34);
    const AV_CX = cX + Math.round(HEAD_H * 0.5);
    const AV_CY = cY + Math.round(HEAD_H * 0.5);
    ...
    if (username) {
      const dispName = username.length > 24 ? username.slice(0, 23) + "…" : username;
      ctx.font = F(Math.round(AV_R * 0.62));
      ctx.fillStyle = TEXT_WHITE;
      ctx.textBaseline = "middle";
      ctx.fillText(dispName, AV_CX + AV_R + 16, AV_CY + 1);
